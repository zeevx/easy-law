...
<div class="main-content">
		<div class="logo_img">
			...
			<h3 class="sho_web d-none d-md-block"> {{ __('auth.Welcome back, Please login') }} <br> {{ __('auth.to your account') }}</h3>
			...
			@csrf
				...
				<input required name="email" id="email" type="text" placeholder="{{ __('auth.Enter email address') }}" autofocus class="">

				<input required name="password" id="password" type="password" placeholder="{{ __('auth.Password') }}" class="" value="">
				...
				<button type="button" class="login-res-btn submitting" style="display:none" disabled>{{ __('auth.Logging in') }}...</button>
			</form>
